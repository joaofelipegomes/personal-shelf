...
interface SettingsModalProps {
  ...
  onUpdate: (newData: { username: string, full_name: string, bg_color: string, avatar_url: string | null }) => void;
...
export const SettingsModal = ({ isOpen, onClose, currentProfile, onUpdate, onPreviewColorChange, showToast, onChangePassword }: SettingsModalProps) => {
  const [formData, setFormData] = useState<{username: string, full_name: string, bg_color: string, avatar_url: string | null}>({
  ...
  useEffect(() => {
    ...
    if (!isOpen || !trimmedUsername || trimmedUsername.length < 3 || trimmedUsername === currentProfile.username.toLowerCase()) {
      setUsernameAvailable(null);
      return;
    }
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="bottom-0 sm:bottom-6 left-0 right-0 sm:left-1/2 sm:-translate-x-1/2 z-[5001] fixed flex flex-col bg-white shadow-2xl border border-black/5 rounded-t-[48px] sm:rounded-[48px] w-full sm:w-[500px] max-h-[92vh] sm:max-h-[95vh] overflow-hidden pb-[env(safe-area-inset-bottom)]"
          >
            {/* Apple Sheet Handle */}
            ...
            <form onSubmit={handleSubmit} className="space-y-6 px-8 pb-8">
              <div className="flex flex-col items-center gap-3 mb-6">
                <div className="relative group">
                  {formData.avatar_url ? (
                    <>
                      <img src={formData.avatar_url} alt="Avatar" className="w-24 h-24 rounded-full object-cover shadow-sm border border-gray-200" />
                      <button 
                        type="button" 
                        onClick={() => { setFormData(prev => ({ ...prev, avatar_url: null })); if (fileInputRef.current) fileInputRef.current.value = ''; }} 
                    ...
                    <label className="w-24 h-24 rounded-full bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-center text-gray-400 border border-gray-200 cursor-pointer overflow-hidden group">
                      <svg width="32" height="32" viewBox="0 0 24 24" fill="none" className="transform group-hover:scale-110 transition-transform">
                ...
                <div className="group relative">
                  <input
                    ...
                    className={`bg-gray-50 p-4 border rounded-2xl outline-none focus:ring-2 focus:ring-black w-full transition-all text-sm font-medium ${
                      usernameAvailable === false ? 'border-red-200' : 'border-gray-100'
                    }`}
                  ...
                  <div className="top-1/2 right-4 absolute flex items-center gap-2 -translate-y-1/2 pointer-events-none">
                    ...
                    ) : (formData.username.trim().toLowerCase() !== currentProfile.username.toLowerCase() && formData.username.length >= 3) && (
              ...
              <div className="space-y-3">
                <span className="text-sm font-bold text-black px-1">Segurança</span>
                <button
                  type="button"
