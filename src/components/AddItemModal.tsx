...
import type { ShelfItem, ItemType, FontFamily } from '../types/item';
...
export const AddItemModal = ({ isOpen, onClose, onAdd, onDelete, initialData, buttonColor = '#000000', defaultType = 'image' }: AddItemModalProps) => {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="bottom-0 sm:bottom-6 left-0 right-0 sm:left-1/2 sm:-translate-x-1/2 z-[2001] fixed flex flex-col bg-white shadow-2xl border border-black/5 rounded-t-[48px] sm:rounded-[48px] w-full sm:w-[500px] max-h-[92vh] sm:max-h-[95vh] overflow-hidden pb-[env(safe-area-inset-bottom)]"
          >
            {/* Apple Sheet Handle */}
            ...
            <form onSubmit={handleSubmit} className="flex-1 space-y-8 p-6 pt-6 overflow-y-auto">
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder={formData.type === 'text' ? "O que você quer dizer?" : "O que você quer adicionar?"}
                />
              </div>

              {formData.type === 'text' && (
                <div className="space-y-4">
                  <label className="text-gray-400 text-xs font-bold uppercase px-1">Fonte</label>
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              ...
                              else if (currentRating === star - 0.5) { if (star === 1) newRating = 0; else newRating = star; }
                              else if (currentRating === 0 && star === 1) { newRating = 1; }
                  ...
                  {!formData.isAutoRotation && (
                    <motion.div initial={{ opacity: 0, height: 0, scale: 0.95 }} animate={{ opacity: 1, height: 'auto', scale: 1 }} exit={{ opacity: 0, height: 0, scale: 0.95 }} className="overflow-hidden">
                      <div className="flex justify-around items-end gap-4 bg-gray-50 mt-2 p-4 border border-gray-100 rounded-2xl">
                        <button type="button" onClick={() => setFormData(prev => ({ ...prev, rotationType: 'left' }))} className="group flex flex-col items-center gap-2 cursor-pointer">
                        ...
                        <button type="button" onClick={() => setFormData(prev => ({ ...prev, rotationType: 'center' }))} className="group flex flex-col items-center gap-2 cursor-pointer">
                        ...
                        <button type="button" onClick={() => setFormData(prev => ({ ...prev, rotationType: 'right' }))} className="group flex flex-col items-center gap-2 cursor-pointer">
