			...
			{isOpen && (
				<>
					...
					<motion.div
						...
						className="bottom-[max(0.625rem,env(safe-area-inset-bottom))] left-2.5 right-2.5 sm:bottom-0 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-[6001] fixed flex flex-col bg-white shadow-2xl border border-black/5 rounded-[32px] sm:rounded-t-[48px] sm:rounded-b-none w-auto sm:w-[450px] max-h-[92dvh] sm:max-h-[95dvh] overflow-hidden"
					>
						{/* Handle */}
						...
						<form onSubmit={handleSubmit} className="space-y-4 px-8 pb-8">
							...
							<button
								disabled={loading}
								type="submit"
								className="w-full shadow-black/10 shadow-lg border border-black/5 mt-6 py-4 rounded-xl font-bold text-sm hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer disabled:bg-gray-400 disabled:opacity-50"
