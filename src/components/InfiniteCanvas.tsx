...
export const InfiniteCanvas = ({ username }: InfiniteCanvasProps) => {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[100dvh] overflow-hidden transition-colors duration-500 touch-none" style={{ backgroundColor: currentBgColor }}>
      ...
      <div className="top-6 right-6 left-6 z-50 fixed flex justify-between items-start pointer-events-none">
        ...
        <div ref={menuRef} className="flex flex-col items-start gap-2 pointer-events-auto">
          <button 
            ...
            className={`bg-white/80 backdrop-blur-md px-4 py-2 rounded-2xl shadow-sm border border-black/5 flex items-center justify-center gap-2 transition-all ${isOwner ? 'hover:bg-gray-50 cursor-pointer active:scale-95' : 'cursor-default'}`}
          >
            {isOwner && (
      ...
      <div className="right-8 bottom-8 z-[1000] fixed flex flex-col gap-3">
        {isOwner && (
          <button
            onClick={() => {
            ...
            className="flex justify-center items-center bg-white shadow-sm shadow-xl border border-black/5 rounded-full w-14 h-14 hover:scale-110 active:scale-95 transition-all duration-200 cursor-pointer"
