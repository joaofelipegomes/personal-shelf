...
export const InfiniteCanvas = ({ username }: InfiniteCanvasProps) => {
  ...
  const [profileData, setProfileData] = useState<{id: string, username: string, full_name: string | null, bg_color?: string, avatar_url?: string | null} | null>(null);
  ...
  useEffect(() => {
    const zoomTowardsPoint = (clientX: number, clientY: number, newScale: number) => {
  ...
  const addItem = async (data: any) => {
    ...
    if (editingItem) {
      ...
      const { error } = await supabase.from('shelf_items').update(updateData).eq('id', editingItem.id);
      if (error) {
  ...
  const handleToggleLike = async (item: ShelfItem) => {
    ...
    try {
      ...
      setItems(prev => prev.map(i => {
        if (i.id === item.id) {
          return { ...i, likesCount: (i.likesCount || 0) + (isLiked ? -1 : 1) };
        }
        return i;
      }));
    } catch (err) {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[100dvh] overflow-hidden transition-colors duration-500 touch-none" style={{ backgroundColor: currentBgColor }}>
      ...
      <div className="top-6 right-6 left-6 z-50 fixed flex justify-between items-start pointer-events-none">
        {/* Lado Esquerdo: Menu do Perfil */}
        <div ref={menuRef} className="flex flex-col items-start gap-2 pointer-events-auto">
          <button 
          ...
          >
            {profileData?.avatar_url ? (
              <img src={profileData.avatar_url} alt={displayName} className="w-6 h-6 rounded-full object-cover shadow-sm border border-black/10" />
            ) : (
        ...
        <div className="pointer-events-auto">
          {isOwner && (
            <button
              ...
              title="Compartilhar Prateleira"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" className="text-black">
      ...
      >
        ...
        >
          <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle, rgba(0,0,0,0.08) 1.5px, transparent 1.5px)', backgroundSize: '40px 40px' }} />
          ...
          {items.map((item) => (
            <ProjectCard 
              ...
              isDimmed={interactionMenuId !== null && interactionMenuId !== item.id}
      ...
      <div className="left-6 bottom-8 z-[1000] fixed flex flex-col items-start gap-3 pointer-events-none">
        <AnimatePresence>
          {zoomDisplay !== 100 && (
            <motion.button 
      ...
      <div className="right-8 bottom-8 z-[1000] fixed flex flex-col items-end gap-3">
        {isOwner && (
          <button
            onClick={() => {
